...
Also split the current window when staring a REPL."
  ...
  (let* (window-script
    ...
    (when (not (eir--matching-elements
      ...
      (message window-script)
      (message name-script-buffer)
      ...
      (select-frame-by-name window-script)
      (select-frame-by-name name-script-buffer)
      
      (select-window name-script-buffer))))
...
and execute by FUN-EXECUTE."
  ...
  (let* (;; Assign the current buffer
	 ...
	 (script-frame (selected-frame))
	 (script-buffer (buffer-name))
	 )
    ...
    ;; ;; (message (type-of script-window))
    ;; ;; (message (type-of script-frame))

    ;; ;; (message "before switch back")

    ;; ;; (select-window script-window)
    ;; ;; (select-frame script-frame)
    ;; ;; (select-frame script-window)
    
    ;; ;; (select-frame-by-name script-window)
    
    ;; (switch-to-buffer-other-frame script-buffer)
    ;; ;; (message "after switch back")
